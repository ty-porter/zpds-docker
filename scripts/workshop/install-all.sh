...
ITEMS_FILE=$SERVER_DIR/workshop/items.txt
...
while IFS= read -r line; do
  ...
  id="${id// /}"     # strip spaces
  [[ -z "$id" ]] && continue
  workshop_args+=(+workshop_download_item ${CLIENT_APP_ID} ${id})
done < "$ITEMS_FILE"
...
for item_dir in ${SERVER_DIR}/steamapps/workshop/content/${CLIENT_APP_ID}/*/; do
...
for map in $(find ${SERVER_DIR}/zp/maps -name "*.bsp" -exec basename {} .bsp \;); do
  grep -qxF "$map" ${SERVER_DIR}/zp/addons/amxmodx/configs/maps.ini || echo "$map" >> ${SERVER_DIR}/zp/addons/amxmodx/configs/maps.ini
done
